...
export const Route = createFileRoute("/start")({
  head: () => ({
    meta: [
      {
        title: "Make Money Helping Businesses Get Online — Kodarai",
      },
      {
        name: "description",
        content:
...
const EXPERIENCE_OPTIONS = [
  ...
  },
  {
    ...
    description: "I want more customers.",
    icon: Users,
  },
];
...
const SITUATION_OPTIONS = [
  ...
  },
  {
    ...
    description: "I need a better way to show them what I can do.",
    icon: MessageCircle,
  },
  {
...
function StartPage() {
  ...
  const saveAnswers = (nextAnswers: FunnelAnswers) => {
  ...
  const chooseSituation = (value: Situation) => {
    const next = {
      ...answers,
      situation: value,
    };
          ...
          >
            ...
            {showWhy && (
              <div className="mt-4 rounded-2xl border border-[#dfe6dd] bg-white p-4 text-sm leading-6 text-[#5b655f] shadow-sm">
                Many businesses still need a better website. You can find one,
                create something useful for them, show the owner and agree on a
          ...
          >
            ...
            <div className="mt-6 flex gap-3 rounded-2xl bg-[#e6f5e8] p-4">
              ...
              <p className="text-[13px] leading-5 text-[#385344]">
                You don't need many customers. A few website sales can add up
                quickly depending on what you charge.
              </p>
            </div>
          </QuestionShell>
        )}
        ...
        {step === 3 && (
          ...
          >
            <div className="space-y-3">
              {SITUATION_OPTIONS.map((option) => (
                <ChoiceCard
                  ...
                  description={option.description}
                  icon={option.icon}
                  selected={answers.situation === option.value}
            ...
            <button
              type="button"
              onClick={() =>
                chooseSituation(answers.situation ?? "no_idea")
              }
...
function Brand() {
  return (
    ...
    >
      Kodar
      <span className="text-[#11a55b]">AI</span>
    </a>
  );
}
  ...
  return (
    <main className="flex min-h-[100dvh] flex-col px-5 pb-7 pt-6 sm:px-7">
      ...
      <h1 className="mt-4 text-[43px] font-bold leading-[0.98] tracking-[-0.055em] sm:text-[50px]">
        Make money helping Nigerian businesses get online.
      </h1>

      <p className="mt-4 max-w-[440px] text-[15px] leading-6 text-[#4f5953]">
        You don't need to be a professional web designer. Kodarai helps you
        find businesses that need a website, create one for them, and contact
        the owner.
      </p>
      ...
      <button
        type="button"
        onClick={onStart}
        className="mt-5 flex min-h-[56px] w-full items-center justify-center gap-2 rounded-xl bg-[#076b3a] px-5 text-[15px] font-semibold text-white shadow-[0_8px_20px_rgba(6,83,48,0.16)] transition active:scale-[0.99]"
      >
      ...
      <p className="mt-3 text-center text-[11px] text-[#7b827d]">
        Free to get started · Takes 30 seconds
      </p>
      ...
      >
        Why this works
        <ChevronDown
          className={`size-4 transition ${showWhy ? "rotate-180" : ""}`}
        />
      </button>

      {showWhy && (
        <div className="mt-3 rounded-2xl border border-[#dfe4dd] bg-white p-4 text-sm leading-6 text-[#5a645e] shadow-sm">
          The model is simple: find a business that needs a better website,
          make something useful to show them, contact the owner, then agree on
          a price if they want to work with you.
        </div>
      )}

      <div className="relative mt-auto overflow-hidden pt-8">
        <div className="relative mx-auto max-w-[380px] overflow-hidden rounded-[28px] bg-[#dce8dc] p-5">
          <div className="absolute -right-10 -top-8 size-36 rounded-full bg-[#c2dfc6]" />
          <div className="absolute -bottom-10 -left-8 size-32 rounded-full bg-[#c5d9c4]" />

          <div className="relative flex items-end justify-between gap-4">
            <div className="max-w-[210px] pb-2">
              <p className="text-[13px] font-semibold text-[#1a422b]">
                Real skills.
                <br />
                Real businesses.
                <br />
              ...
              <div className="mt-4 flex items-center gap-2 text-xs font-medium text-[#197346]">
            ...
            <div className="relative h-[145px] w-[145px] shrink-0">
              <div className="absolute bottom-0 right-0 h-[110px] w-[130px] rounded-t-[55px] bg-[#0b6038]" />

              <div className="absolute right-[45px] top-0 size-[52px] rounded-full bg-[#8a5b39]" />

              <div className="absolute right-[38px] top-[38px] h-[60px] w-[68px] rounded-t-[32px] bg-[#11462f]" />

              <div className="absolute bottom-1 left-0 h-[55px] w-[110px] rotate-[-4deg] rounded-lg border border-black/10 bg-[#e9ece7] shadow-md">
                <div className="mx-auto mt-4 h-2 w-2 rounded-full bg-black/15" />
              </div>
            </div>
          </div>
        </div>
...
function Benefit({ text }: { text: string }) {
  return (
    <div className="flex items-center gap-2.5 py-1">
      ...
      <span className="text-[13px] font-medium text-[#2a4131]">{text}</span>
    </div>
  );
}
...
function QuestionShell({
  ...
  return (
    <main className="min-h-[100dvh] px-5 pb-10 pt-5 sm:px-7">
      ...
      <p className="mt-3 text-[14px] leading-5 text-[#5f6862]">{subtitle}</p>
  ...
  return (
    ...
    >
      ...
      <span className="flex-1">
        ...
        <span className="text-xs text-[#727b75]">per month</span>
      </span>
...
function Result({
  ...
  const goal = Number(answers.goal ?? "250000");
  ...
  const examples = useMemo(() => {
    const firstPrice = 50_000;
    const secondPrice = 100_000;

    const firstClients = Math.ceil(goal / firstPrice);
    const secondClients = Math.ceil(goal / secondPrice);

    return {
      firstPrice,
      firstClients,
      firstTotal: firstPrice * firstClients,
      secondPrice,
      secondClients,
      secondTotal: secondPrice * secondClients,
    };
  }, [goal]);
  ...
  return (
    <main className="min-h-[100dvh] px-5 pb-10 pt-5 sm:px-7">
      <button
        type="button"
        onClick={onBack}
        className="flex size-9 items-center justify-center rounded-full text-[#1c251f] hover:bg-black/5"
      >
        <ArrowLeft className="size-5" />
      </button>
      ...
      <h1 className="mt-2 text-[33px] font-bold leading-[1.02] tracking-[-0.05em]">
        Let's work towards{" "}
        <span className="text-[#0a9a51]">{goalLabel}</span> a month.
      </h1>
      ...
      <div className="mt-6 overflow-hidden rounded-2xl border border-[#e2e6e1] bg-white shadow-[0_3px_14px_rgba(25,35,28,0.04)]">
        <IncomeExample
          clients={examples.firstClients}
          price={examples.firstPrice}
          total={examples.firstTotal}
        />

        <div className="mx-4 h-px bg-[#e9ece8]" />

        <IncomeExample
          clients={examples.secondClients}
          price={examples.secondPrice}
          total={examples.secondTotal}
        />
      </div>
      ...
      <button
        type="button"
        onClick={onStart}
        className="mt-8 flex min-h-[56px] w-full items-center justify-center gap-2 rounded-xl bg-[#076b3a] px-5 text-[15px] font-semibold text-white shadow-[0_8px_20px_rgba(6,83,48,0.16)] active:scale-[0.99]"
      >
      ...
      <p className="mt-4 text-[10px] leading-4 text-[#818982]">
        Earnings are not guaranteed. These numbers are examples only. What you
        earn depends on your effort, pricing and ability to find and close
        customers.
      </p>
    </main>
  );
    ...
    <div className="flex items-center justify-between gap-3 p-4">
      <div>
        <p className="text-[13px] font-semibold">
          Sell {clients} {clients === 1 ? "website" : "websites"} at ₦
          {price.toLocaleString("en-NG")}
        </p>

        <p className="mt-1 text-[11px] text-[#747c77]">
          {clients} × ₦{price.toLocaleString("en-NG")}
        </p>
      </div>
