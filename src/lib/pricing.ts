...
import { PRICING_PLATFORM_FEE_NGN } from "@/lib/numbers";
...
export function estimateWebsitePrice(
  ...
  const band =
    findPriceBand(category);

  const businessSize =
    estimateBusinessSize({
      ...lead,
      category,
    });
  ...
  if (!lead.hasWebsite) {
    ...
    reasons.push(
      "Existing website creates a redesign opportunity",
    );
  }
  ...
  if (
    ...
    reasons.push(
      `${lead.rating.toFixed(1)}\u2605 rating with ${lead.reviewCount.toLocaleString()} reviews`,
    );
  }
  ...
  if (
    ...
    reasons.push(
      `${businessSize.toLowerCase()} business signal`,
    );
  }
  ...
  if (
    band.complexity ===
      "Advanced" ||
    band.complexity === "Premium"
  ) {
    reasons.push(
      `${band.complexity.toLowerCase()} website requirements`,
    );
  }
...
/**
 * Naira symbol is escaped deliberately.
 * This prevents the symbol from being corrupted if the source
 * file passes through a bad text encoding.
 */
const NAIRA = "\u20A6";
...
export function calculateCustomerPrice(
  ...
  if (
    ...
    throw new Error(
      "Invalid provider price or FX rate",
    );
  }
