...
function token(): string {
  ...
  if (!value) throw new Error("Vercel is not configured. Ask an administrator to set VERCEL_TOKEN.");
  return value;
}
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) throw new Error(payload.error?.message || payload.message || `Vercel API failed (${response.status}).`);
  return payload;
}

function projectName(slug: string): string {
  return `kodarai-${slug}`.toLowerCase().replace(/[^a-z0-9-]/g, "-").slice(0, 52);
}

export async function createVercelProject(slug: string, existingId?: string | null, existingName?: string | null): Promise<VercelProject> {
  if (existingId && existingName) return { id: existingId, name: existingName };
  const name = projectName(slug);
  try {
    const project = await request<VercelProject>("/v9/projects", {
      method: "POST",
    ...
    // A retry after a network interruption may find a project created by the first request.
...
async function uploadFile(file: StudioFile): Promise<{ file: string; sha: string; size: number }> {
  ...
  if (!response.ok) {
    ...
    throw new Error(payload.error?.message || payload.message || `Vercel file upload failed (${response.status}).`);
  }
  return { file: file.path, sha, size: bytes.byteLength };
}

export async function deployToVercel(input: { projectName: string; files: StudioFile[] }): Promise<VercelDeployment> {
  if (!input.files.some((file) => file.path === "index.html")) throw new Error("A website project needs index.html before it can be published.");
  const files = await Promise.all(input.files.map(uploadFile));
  return request<VercelDeployment>("/v13/deployments?forceNew=1", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ name: input.projectName, files, target: "production", projectSettings: { framework: null } }),
  });
}
...
export async function getVercelDeploymentLogs(id: string): Promise<string> {
  const logs = await request<Array<{ text?: string }>>(`/v3/deployments/${encodeURIComponent(id)}/events?direction=backward&limit=30`);
  return logs.map((entry) => entry.text).filter(Boolean).join("\n").slice(-4000);
}

export async function waitForVercelDeployment(id: string, timeoutMs = 95_000): Promise<VercelDeployment> {
  const started = Date.now();
  let latest = await getVercelDeployment(id);
  while (Date.now() - started < timeoutMs) {
    if (latest.readyState === "READY") return latest;
